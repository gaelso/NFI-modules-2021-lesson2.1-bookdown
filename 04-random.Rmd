...
Complete simple random sampling (SRS) designs have a number of disadvantages and are generally not recommended at a national scale. We will still relocate our ```r nplot3_total$n``` plots randomly to compare different sampling designs. 


## Demo {-}

The equivalent of `st_make_grid()` for random sampling is `st_sample()`. Every time `st_sample()` is used a new set of random locations is created. To get the same random allocation every time we run the code, we need to set a "seed" and the function `set.seed()`. In the example we will use `set.seed(10)` but feel free to change or remove at will the chose seed number in the practice below.
...
Well done! In the next section, will compare the plot distribution between random and systematic sampling.
